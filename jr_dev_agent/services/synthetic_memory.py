                ...
                if os.path.exists(features_root):
                    for feature_name in os.listdir(features_root):
                        ...
                        if os.path.exists(ticket_dir):
                            ...
                            # Ensure defaults
                            if "ticket_id" not in summary_data:
                                summary_data["ticket_id"] = ticket_id
                            if "feature_id" not in summary_data:
                                summary_data["feature_id"] = feature_name

                            # Add new fields
                            if change_required:
                                summary_data["change_required"] = change_required
                            if changes_made:
                                summary_data["changes_made"] = changes_made
                            if pr_url:
                                summary_data["pr_url"] = pr_url
